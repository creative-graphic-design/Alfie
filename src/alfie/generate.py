...
def get_pipe(image_size, scheduler, device):
    ...
    dpm_scheduler = DPMSolverMultistepScheduler.from_pretrained(
        pipeline_key, subfolder="scheduler"
    )
    if scheduler == "euler":
...
def base_arg_parser():
    ...
    parser.add_argument(
        "--scheduler", type=str, default="euler", choices=["euler", "euler_ancestral"]
    )
    ...
    parser.add_argument(
        "--cutout_model", type=str, default="grabcut", choices=["grabcut", "vit-matte"]
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="False",
        help='Add the suffix "on a white background"',
    )
    parser.add_argument("--seed", type=int, default=2024, help="random seed")
    parser.add_argument(
        "--vit_matte_key", type=str, default="hustvl/vitmatte-base-composition-1k"
    )
    parser.add_argument(
        "--nouns_to_exclude",
        nargs="+",
...
def parse_bool_args(args):
    ...
    args.nouns_to_exclude = (
        args.nouns_to_exclude if args.exclude_generic_nouns else None
    )
