...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        attention_scores = attn.get_attention_scores(
    ...
    def compute_global_cross_heat_map(
        self, prompt: str, layers: List[int] = None, normalize: bool = False
    ) -> CrossGlobalHeatMap:
        """
        ...
        with auto_autocast(device_type=heat_maps.device_type, dtype=torch.float32):
            for (layer), heat_map in heat_maps:
                if layer in layers:
                    ...
                    all_merges.append(
                        F.interpolate(
                            heat_map, size=(self.h, self.w), mode="bicubic"
                        ).clamp_(min=0)
                    )
            ...
            if normalize:
                maps = maps / (
                    maps[:-1].sum(0, keepdim=True) + 1e-6
                )  # drop out[PAD] for proper probabilities
