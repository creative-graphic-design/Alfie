...
class SelfGlobalHeatMap:
    ...
    def scale_correction(self, pixel_ids):
        ...
        """
        pixel_ids_2d = [
            (p_id // self.latent_w, p_id % self.latent_w) for p_id in pixel_ids
        ]
        inner_pixel_ids_2d = [
            (x // self.scale_factor, y // self.scale_factor) for x, y in pixel_ids_2d
        ]
        scaled_pixel_ids = [x * self.inner_latent_w + y for x, y in inner_pixel_ids_2d]
        return scaled_pixel_ids

    def compute_pixel_heat_map(
        self, latent_pixels: Union[List[int], int]
    ) -> SelfPixelHeatMap:
        """
    ...
    def compute_bbox_heat_map(
        ...
        if x2 < x1 or y2 < y1:
            raise Exception(
                "Enter valid bounding box! (x1,y1) is the top-left corner and (x2,y2) is the bottom-right corner."
            )
        pix_ids = [
            x
            for y in range(y1, y2 + 1)
            for x in range((self.latent_w * y) + x1, (self.latent_w * y) + x2 + 1)
            if x < (self.latent_h * self.latent_w)
        ]
        # scale correction
    ...
    def inner_pixel_ids(
        self, pts: Union[List[List[int]], List[int]], image_h: int, image_w: int
    ) -> List:
        """
        ...
        if image_h != image_w:
            raise Exception(
                "Non-Square images not supported yet! `image_h` should be equal to `image_w"
            )
    ...
    def compute_contour_heat_map(
        self,
        pts: Union[List[List[int]], List[int]],
        image_h: int,
        image_w: int,
        guide_heatmap: Optional[torch.tensor] = None,
    ) -> SelfPixelHeatMap:
        """
        pts should be a represent a single polygon multi-piece polygon is not handled by this function, check out `segmentation_heat_map`
        pts should be be a list of [x,y] coordinates of the contour
        ...
        if guide_heatmap is None:
            ...
            pix_weights = torch.tensor(
                [
                    guide_heatmap[pix_id // self.latent_w, pix_id % self.latent_w]
    ...
    def compute_segmentation_heat_map(
        self,
        ...
        if guide_heatmap is None:
            ...
            pix_weights = torch.tensor(
                [
                    guide_heatmap[pix_id // self.latent_w, pix_id % self.latent_w]
                    for pix_id in segments_inner_pixs
                ]
            )[:, None, None]
            pix_weights = pix_weights.cpu()
            heatmap = torch.zeros((self.latent_h, self.latent_w)).cpu()
    ...
    def compute_guided_heat_map(self, guide_heatmap: torch.tensor):
        ...
        conv_weight = (
            guide_heatmap.to(heat_maps2d.device)
            .view(-1, self.latent_h * self.latent_w)
            .clone()[:, :, None, None]
        )
        ...
        if heat_maps2d.dtype == torch.float16 or conv_weight.dtype == torch.float16:
            ...
            guided_heatmap = F.conv2d(heat_maps2d.cuda(), conv_weight.cuda())[
                0, 0
            ].cpu()
        else:
            # Aggregating all the heatmaps using convolution operation i.e. weighted average using `guide_heatmap` weights
    ...
    def compute_pixel_diffused_heat_map(
        self,
        latent_pixel_id: int,
    ...
    def compute_diffused_heat_map(
        self, method: str = "thresholding", n_iter: int = 20, thresh: int = 0.02
    ):
        """
        ...
        # weight of the convolution layer that performs attention diffusion (making a copy to prevent changing the heatmap)
        conv_weight = self.heat_maps.view(-1, self.latent_h * self.latent_w).clone()[
            :, :, None, None
        ]
        ...
        if method == "thresholding":
            ...
            # By setting them to 0
            conv_weight[
                conv_weight < (torch.min(conv_weight, 1, keepdims=True)[0] + thresh)
            ] = 0
            ...
            for _ in tqdm(range(n_iter)):
                ...
                conv_weight = F.conv2d(heat_maps2d, conv_weight)[0].view(
                    self.heat_maps.shape[0], self.heat_maps.shape[0]
                )[:, :, None, None]
        ...
        return SelfGlobalHeatMap(
            conv_weight.view(self.heat_maps.shape[0], self.latent_h, self.latent_w),
